...
@interface okbSDK ()<GCDAsyncSocketDelegate>
{
...
@implementation okbSDK

+(instancetype)shareInstance
{
    static okbSDK * instance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[okbSDK alloc] init];
    });
    return instance;
}
...
-(void)connectTcpServer:(int)port
{
    ...
    NSError *err;
    [gcdSocket connectToHost:@"127.0.0.1" onPort:port error:&err];
    if (err) {
#if IS_OPEN_DEBUG
...
{
#if IS_OPEN_DEBUG
    if (messageType == ZTcpSendMessageText) {
        NSString * content = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
        NSLog(@"tcp发送一次文本内容:%@",content);
    }else if (messageType == ZTcpSendMessagePicture)
    {
...
-(void)socket:(GCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag
{
    ...
    //数据回调
    ZTcpSendMessageType messageType = (ZTcpSendMessageType)[headDic[kHeadMessageType] integerValue];
